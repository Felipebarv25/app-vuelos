    ...
    <button
      ...
      className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition ${
        activo
          ...
          : "bg-white text-marca-900 ring-1 ring-slate-200 hover:ring-marca-300 dark:bg-slate-800 dark:text-marca-300 dark:ring-slate-600 dark:hover:ring-marca-700"
      }`}
    >
      {children}
    </button>
...
export function Boton({ onClick, href, variante = "primario", children, style }) {
  ...
  const estilos = {
    ...
    sec: "bg-white text-marca-600 ring-[1.5px] ring-marca-100 hover:bg-marca-50 dark:bg-slate-800 dark:text-marca-400 dark:ring-slate-700 dark:hover:bg-slate-700",
...
export function Tarjeta({ children, style }) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-suave dark:border-slate-700 dark:bg-slate-800" style={style}>
      {children}
    </div>
  );
}
