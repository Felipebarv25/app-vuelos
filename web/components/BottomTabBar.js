...
// Los rotulos iban en español a fuego, asi que un usuario en ingles veia
// "Inicio · Destinos · Ofertas · Mis viajes" en la barra inferior. Pasan a
// claves, y las cinco ya existian traducidas: no se anade ni una cadena
// nueva al fichero de idiomas.
const TABS = [
  { href: "/",            clave: "inicio",       icono: "home" },
  { href: "/destino",     clave: "navDestinos",  icono: "globe" },
  { href: "/mi-viaje",    clave: "navMiViaje",   icono: "compass" },
  { href: "/ofertas",     clave: "navOfertas",   icono: "plane" },
  { href: "/mis-viajes",  clave: "navMisViajes", icono: "bookmark" },
];
...
export default function BottomTabBar() {
  ...
  return (
    ...
    >
      <div className="flex">
        {TABS.map((tab) => {
          const activo = esActivo(pathname, tab.href);
          return (
            <Link
              key={tab.href}
              href={tab.href}
...
function TabIcono({ nombre }) {
  ...
  if (nombre === "compass") return <svg {...p}><circle cx="12" cy="12" r="10" /><path d="m16.24 7.76-2.12 6.36-6.36 2.12 2.12-6.36z" /></svg>;
