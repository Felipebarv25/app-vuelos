...
export default function InteligenciaViaje({ ruta, analisis, decisiones }) {
  ...
  useEffect(() => {
    ...
    const tieneFecha = /^\d{4}-\d{2}-\d{2}$/.test(fecha);
    const tieneMes = /^\d{4}-\d{2}$/.test(mes);
    if ((!tieneFecha && !tieneMes) || paradas.length < 2) { setVuelosVivos({}); setConsultandoVuelos(false); setModoConsultaVuelos(null); return undefined; }
    ...
    if (!tramos.length) { setVuelosVivos({}); setConsultandoVuelos(false); return undefined; }
    setConsultandoVuelos(true);
    setModoConsultaVuelos(tieneFecha ? "fecha" : "mes");
