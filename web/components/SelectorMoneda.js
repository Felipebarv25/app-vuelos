...
export default function SelectorMoneda({ value, onChange, className = "" }) {
  ...
  // Cuando se abre, focus al input y selección visible.
  useEffect(() => {
    if (abierto) {
      setQ("");
      setISeleccion(0);
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {abierto && (
        <div className="absolute right-0 top-full z-30 mt-1 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-modal">
          <input
            ref={inputRef}
            type="text"
            value={q}
