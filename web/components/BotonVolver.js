 ...
 *
 * `alVolver` cambia el COMPORTAMIENTO, no el aspecto. Antes tambien pintaba el
 * boton en teal solido, asi que la misma accion —volver— se veia de dos formas
 * distintas segun la pagina: blanco en /ofertas o /comparar y verde oscuro en
 * /mis-viajes. Un control que hace lo mismo tiene que verse igual en todas
 * partes, asi que el boton es blanco siempre.
 */
export default function BotonVolver({ href = null, etiqueta = "Volver", espaciar = true, alVolver = null }) {
  ...
  return (
    ...
    <div className="fixed left-0 top-[90px] z-[60] px-4 sm:px-6">
      ...
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="transition group-hover:-translate-x-0.5 text-marca-700 dark:text-marca-300">
