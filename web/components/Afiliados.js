...
function CtaReserva({ href, icono, titulo, desc, tipo, color = "marca" }) {
  const tono = {
    marca: "from-marca-50 to-violet-50 border-marca-100 text-marca-700",
    emerald: "from-emerald-50 to-teal-50 border-emerald-100 text-emerald-700",
    sky: "from-sky-50 to-indigo-50 border-sky-100 text-sky-700",
  }[color];
  return (
    <a
      href={href}
    ...
    >
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-xl shadow-suave">
        {icono}
      </span>
...
export function AfiliadosCiudad({ ciudad, t = (k) => k }) {
  ...
  return (
    <div className="mt-3.5 rounded-2xl border border-slate-100 bg-white p-4 shadow-suave">
      <div className="mb-3 text-sm font-bold text-marca-900">🧳 {t("afTitulo")} · {nombre}</div>
...
export function BotonTourLugar({ lugar, ciudad, t = (k) => k }) {
  ...
  return (
    <a
      ...
      className="rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 px-3.5 py-2 text-[13px] font-bold text-white shadow-suave transition hover:brightness-105"
    >
      🎟️ {t("afTourCerca")}
    </a>
  );
