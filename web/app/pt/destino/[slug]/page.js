...
export default async function DestinoPt({ params }) {
  const { slug } = await params;
  const d = getDestinoPorSlug(slug);
  if (!d) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-16 text-center">
        ...
        <NavTop />
        <h1 className="text-3xl font-extrabold">Destino não encontrado</h1>
        <p className="mt-2 text-slate-600"><Link href="/pt" className="text-marca-600 underline">Voltar ao início</Link></p>
      </main>
    );
  }
  ...
  const seo = datosSeoDe(d, "pt");
  const faqs = faqsDe(d, "pt");
  const foto = await fotoCiudad(d.ciudad, d.pais, d.lat, d.lon);

  const jsonLd = { "@context": "https://schema.org", "@type": "TouristDestination", name: nombre, description: `Informações de viagem para ${nombre}.`, url: `${SITIO}/pt/destino/${slug}`, geo: { "@type": "GeoCoordinates", latitude: d.lat, longitude: d.lon }, address: { "@type": "PostalAddress", addressCountry: d.pais } };
  const breadcrumbs = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "Início", item: `${SITIO}/pt` },
    { "@type": "ListItem", position: 2, name: "Destinos", item: `${SITIO}/pt/destino` },
    { "@type": "ListItem", position: 3, name: nombre, item: `${SITIO}/pt/destino/${slug}` },
  ]};
  ...
  return (
    <main className="bg-slate-50">
      ...
      <nav className="mx-auto max-w-4xl px-6 pt-6 text-[13px] text-slate-500">
        <Link href="/pt" className="hover:text-marca-600">Início</Link>
      ...
      {foto?.url ? (
        <header className="relative h-[420px] w-full overflow-hidden sm:h-[480px]">
          ...
          <div className="relative mx-auto flex h-full max-w-4xl flex-col justify-end px-6 pb-8 pt-12 text-white">
            <div className="flex items-center gap-3 text-[13px] font-semibold uppercase tracking-[0.18em] text-white/85">
              <span className="text-3xl">{d.bandera}</span><span>Viaje para {d.pais}</span>
            </div>
            <h1 className="mt-2 text-4xl font-extrabold tracking-tight drop-shadow-md sm:text-5xl">Viagem para {d.ciudad}</h1>
            <Link href={`/?destino=${slug}`} className="mt-4 inline-flex w-fit items-center gap-2 rounded-2xl bg-white px-6 py-3.5 text-base font-bold text-marca-700 shadow-marca transition hover:brightness-105">🗺️ Planejar minha viagem para {d.ciudad}</Link>
          </div>
        </header>
      ) : (
        <header className="mx-auto max-w-4xl px-6 pb-6 pt-6">
          <div className="flex items-center gap-3 text-[14px] font-semibold uppercase tracking-[0.18em] text-marca-500">
            <span className="text-3xl">{d.bandera}</span><span>Viaje para {d.pais}</span><FavToggle slug={slug} />
          </div>
          <h1 className="mt-2 text-4xl font-extrabold tracking-tight text-marca-900 sm:text-5xl">Viagem para {d.ciudad}</h1>
          <Link href={`/?destino=${slug}`} className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-marca-500 to-marca-600 px-6 py-3.5 text-base font-bold text-white shadow-marca transition hover:brightness-105">🗺️ Planejar minha viagem para {d.ciudad}</Link>
        </header>
      )}
      ...
      <section className="mx-auto max-w-4xl px-6 py-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Dato titulo="Voo ida e volta" valor={`US$ ${d.vuelo}`} sub="aprox." />
          <Dato titulo="Custo diário" valor={`US$ ${d.dia}`} sub="por pessoa" />
          <Dato titulo="Dias recomendados" valor={diasSugeridos} sub="ideal" />
          <Dato titulo="Orçamento sugerido" valor={`US$ ${presup}`} sub={`${diasSugeridos} dias, 1 pessoa`} />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-4xl px-6 py-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-marca-100 bg-marca-50/40 p-5">
            <div className="text-[11px] font-bold uppercase tracking-wide text-marca-600">🗓️ Melhor época para viajar</div>
            <div className="mt-1.5 text-base leading-relaxed text-slate-700">{seo.mejorEpoca}</div>
            {seo.evitarEpoca && <div className="mt-2 text-[13px] text-slate-500"><b className="text-amber-700">Evite:</b> {seo.evitarEpoca}.</div>}
          </div>
          <div className="rounded-2xl border border-slate-100 bg-white p-5">
            <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">ℹ️ Dados rápidos</div>
            <dl className="mt-2 grid grid-cols-1 gap-1.5 text-[14px] text-slate-600">
              ...
              <div className="flex gap-2"><dt className="font-semibold text-slate-500">Moeda:</dt><dd>{seo.moneda}</dd></div>
            </dl>
            {seo.dato && <div className="mt-3 rounded-xl bg-amber-50 p-3 text-[13px] leading-relaxed text-amber-800"><b>💡 Sabia?</b> {seo.dato}</div>}
          </div>
        </div>
      </section>
      ...
      {seo.platos?.length > 0 && (
        <section className="mx-auto max-w-4xl px-6 py-6">
          <h2 className="text-2xl font-extrabold tracking-tight text-marca-900">Comida típica de {d.ciudad}</h2>
          <p className="mt-1 text-slate-500">Pratos para experimentar.</p>
      ...
      <section className="mx-auto max-w-4xl px-6 py-6">
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-suave">
          <h2 className="text-xl font-extrabold tracking-tight text-marca-900">Reserve sua viagem para {d.ciudad}</h2>
          <p className="mt-1 text-[14px] text-slate-500">Passeios, hospedagem e voos de parceiros que usamos.</p>
          <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
            <a href={linkTours({ q: d.ciudad, lat: d.lat, lon: d.lon })} target="_blank" rel="sponsored noopener" className="flex items-center gap-3 rounded-2xl border border-marca-100 bg-gradient-to-br from-marca-50 to-marca-100/60 p-3.5 text-marca-700 shadow-suave transition hover:brightness-[0.98]">
              ...
              <div className="min-w-0 flex-1"><div className="text-[14.5px] font-bold leading-tight">Passeios e experiências</div><div className="truncate text-[12.5px] text-slate-500">Ingressos sem fila via GetYourGuide</div></div><span className="text-xl">→</span>
            </a>
            <a href={linkHoteles({ ciudad: d.ciudad, pais: d.pais, lat: d.lat, lon: d.lon })} target="_blank" rel="sponsored noopener" className="flex items-center gap-3 rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-teal-50 p-3.5 text-emerald-700 shadow-suave transition hover:brightness-[0.98]">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-xl shadow-suave">🛏️</span>
              <div className="min-w-0 flex-1"><div className="text-[14.5px] font-bold leading-tight">Onde ficar</div><div className="truncate text-[12.5px] text-slate-500">Compare Booking + Hotellook</div></div><span className="text-xl">→</span>
            </a>
            <a href={linkVuelos({ ciudad: d.ciudad, pais: d.pais })} target="_blank" rel="sponsored noopener" className="flex items-center gap-3 rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-50 to-sky-100 p-3.5 text-sky-700 shadow-suave transition hover:brightness-[0.98]">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-xl shadow-suave">✈️</span>
              <div className="min-w-0 flex-1"><div className="text-[14.5px] font-bold leading-tight">Voos baratos</div><div className="truncate text-[12.5px] text-slate-500">Compare no Aviasales</div></div><span className="text-xl">→</span>
            </a>
          </div>
          <div className="mt-2.5 text-[11px] leading-snug text-slate-400">Alguns links são de afiliados: se você reservar, podemos ganhar uma comissão sem custo extra.</div>
        </div>
      </section>

      <section className="mx-auto max-w-4xl px-6 py-6">
        <h2 className="text-2xl font-extrabold tracking-tight text-marca-900">Perguntas frequentes sobre {d.ciudad}</h2>
      ...
      <section className="mx-auto max-w-4xl px-6 py-10">
        <div className="rounded-3xl bg-gradient-to-br from-marca-600 via-marca-700 to-marca-900 p-8 text-white shadow-media">
          <h2 className="text-2xl font-extrabold sm:text-3xl">Pronto para planejar seu roteiro em {d.ciudad}?</h2>
          <p className="mt-2 max-w-2xl text-white/85">Distribuímos os pontos dia a dia com tempos reais de transporte, fotos e preços ao vivo. Sem instalar nada.</p>
          <Link href={`/?destino=${slug}`} className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-white px-6 py-3.5 text-base font-bold text-marca-700 shadow-marca transition hover:brightness-105">🗺️ Começar minha viagem para {d.ciudad}</Link>
        </div>
      </section>

      <footer className="mx-auto max-w-4xl px-6 pb-10 text-center text-[12px] text-slate-400">Dados do OpenStreetMap e Wikipedia · Preços orientativos em USD.</footer>
    </main>
  );
}
